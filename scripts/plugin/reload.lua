...
local function update_funcs(map)
    ...
    local function update_funcs_frame(level)
        local info = getinfo(level+1, "nSlf")
        if info == nil or info.name == "reload" then
            return
        end
        local f = info.func
        info = nil
        ...
        local i = 1
        while true do
            local name, v = getlocal(level+1, i)
            if name == nil then
                if i > 0 then
                    i = -1
                else
        ...
        return update_funcs_frame(level+1)
    end
...
function reload.reload(list)
    ...
    local ok, result = pcall(reload_list, list)
    if not ok then
        sandbox.clear()
