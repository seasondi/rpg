...
--#end -- not DATE_OBJECT_AFX
 ...
 --[[ THE DATE OBJECT METHODS ]]--
  ...
  function dobj:get_date()  local y, m, d = breakdaynum(self.daynum) return y, m+1, d end
  function dobj:get_time()  return breakdayfrc(self.dayfrc) end

  function dobj:get_clock_hour() local h = self:gethours() return h>12 and mod(h,12) or (h==0 and 12 or h) end

  function dobj:get_year_day() return yearday(self.daynum) + 1 end
  function dobj:get_week_day() return weekday(self.daynum) + 1 end   -- in lua weekday is sunday = 1, monday = 2 ...

  function dobj:get_year()   local r,_,_ = breakdaynum(self.daynum)  return r end
  function dobj:get_month() local _,r,_ = breakdaynum(self.daynum)  return r+1 end-- in lua month is 1 base
  function dobj:get_day()   local _,_,r = breakdaynum(self.daynum)  return r end
  function dobj:get_hours()  return mod(floor(self.dayfrc/TICKSPERHOUR),HOURPERDAY) end
  function dobj:get_minutes()  return mod(floor(self.dayfrc/TICKSPERMIN), MINPERHOUR) end
  function dobj:get_seconds()  return mod(floor(self.dayfrc/TICKSPERSEC ),SECPERMIN)  end
  function dobj:get_fracsec()  return mod(floor(self.dayfrc/TICKSPERSEC ),SECPERMIN)+(mod(self.dayfrc,TICKSPERSEC)/TICKSPERSEC) end
  function dobj:get_ticks(u)  local x = mod(self.dayfrc,TICKSPERSEC) return u and ((x*u)/TICKSPERSEC) or x  end

  function dobj:get_week_number(wdb)
  ...
  function dobj:get_iso_weekday() return mod(weekday(self.daynum)-1,7)+1 end   -- sunday = 7, monday = 1 ...
  function dobj:get_iso_weeknumber() return (isowy(self.daynum)) end
  function dobj:get_iso_year() return isoy(self.daynum)  end
  function dobj:get_iso_date()
    local w, y = isowy(self.daynum)
    return y, w, self:getisoweekday()
  end
  function dobj:set_iso_year(y, w, d)
    local cy, cw, cd = self:get_iso_date()
  ...
  function dobj:set_iso_weekday(d)    return self:set_iso_year(nil, nil, d) end
  function dobj:set_iso_weeknumber(w,d)  return self:set_iso_year(nil, w, d)  end

  function dobj:set_year(y, m, d)
  ...
  function dobj:set_month(m, d)return self:set_year(nil, m, d) end
  function dobj:set_day(d)    return self:set_year(nil, nil, d) end

  function dobj:set_hours(h, m, s, t)
  ...
  function dobj:set_minutes(m,s,t)  return self:set_hours(nil,   m,   s, t) end
  function dobj:set_seconds(s, t)  return self:set_hours(nil, nil,   s, t) end
  function dobj:set_ticks(t)    return self:set_hours(nil, nil, nil, t) end

  function dobj:span_ticks()  return (self.daynum*TICKSPERDAY + self.dayfrc) end
  function dobj:span_seconds()  return (self.daynum*TICKSPERDAY + self.dayfrc)/TICKSPERSEC  end
  function dobj:span_minutes()  return (self.daynum*TICKSPERDAY + self.dayfrc)/TICKSPERMIN  end
  function dobj:span_hours()  return (self.daynum*TICKSPERDAY + self.dayfrc)/TICKSPERHOUR end
  function dobj:span_days()  return (self.daynum*TICKSPERDAY + self.dayfrc)/TICKSPERDAY  end

  function dobj:add_years(y, m, d)
    local cy, cm, cd = breakdaynum(self.daynum)
  ...
  function dobj:add_months(m, d)
    return self:add_years(nil, m, d)
  end
  ...
  function dobj:add_days(n)  return dobj_adddayfrc(self,n,TICKSPERDAY,1) end
  function dobj:add_hours(n)  return dobj_adddayfrc(self,n,TICKSPERHOUR,HOURPERDAY) end
  function dobj:add_minutes(n)  return dobj_adddayfrc(self,n,TICKSPERMIN,MINPERDAY)  end
  function dobj:add_seconds(n)  return dobj_adddayfrc(self,n,TICKSPERSEC,SECPERDAY)  end
  function dobj:add_ticks(n)  return dobj_adddayfrc(self,n,1,TICKSPERDAY) end
  local tvspec = {
    ...
    ['%b']=function(self) return sl_months[self:get_month() - 1 + 12] end,
    -- Full month name (December)
    ['%B']=function(self) return sl_months[self:get_month() - 1] end,
    -- Year/100 (19, 20, 30)
    ['%C']=function(self) return fmt("%.2d", fix(self:get_year()/100)) end,
    -- The day of the month as a number (range 1 - 31)
    ['%d']=function(self) return fmt("%.2d", self:get_day())  end,
    -- year for ISO 8601 week, from 00 (79)
    ['%g']=function(self) return fmt("%.2d", mod(self:get_iso_year() ,100)) end,
    -- year for ISO 8601 week, from 0000 (1979)
    ['%G']=function(self) return fmt("%.4d", self:get_iso_year()) end,
    -- same as %b
    ['%h']=function(self) return self:fmt0("%b") end,
    -- hour of the 24-hour day, from 00 (06)
    ['%H']=function(self) return fmt("%.2d", self:get_hours()) end,
    -- The  hour as a number using a 12-hour clock (01 - 12)
    ['%I']=function(self) return fmt("%.2d", self:get_clock_hour()) end,
    -- The day of the year as a number (001 - 366)
    ['%j']=function(self) return fmt("%.3d", self:get_year_day())  end,
    -- Month of the year, from 01 to 12
    ['%m']=function(self) return fmt("%.2d", self:get_month())  end,
    -- Minutes after the hour 55
    ['%M']=function(self) return fmt("%.2d", self:get_minutes())end,
    -- AM/PM indicator (AM)
    ['%p']=function(self) return sl_meridian[self:get_hours() > 11 and 1 or -1] end, --AM/PM indicator (AM)
    -- The second as a number (59, 20 , 01)
    ['%S']=function(self) return fmt("%.2d", self:get_seconds())  end,
    -- ISO 8601 day of the week, to 7 for Sunday (7, 1)
    ['%u']=function(self) return self:get_iso_weekday() end,
    -- Sunday week of the year, from 00 (48)
    ['%U']=function(self) return fmt("%.2d", self:get_week_number()) end,
    -- ISO 8601 week of the year, from 01 (48)
    ['%V']=function(self) return fmt("%.2d", self:get_iso_weeknumber()) end,
    -- The day of the week as a decimal, Sunday being 0
    ['%w']=function(self) return self:get_week_day() - 1 end,
    -- Monday week of the year, from 00 (48)
    ['%W']=function(self) return fmt("%.2d", self:get_week_number(2)) end,
    -- The year as a number without a century (range 00 to 99)
    ['%y']=function(self) return fmt("%.2d", mod(self:get_year() ,100)) end,
    -- Year with century (2000, 1914, 0325, 0001)
    ['%Y']=function(self) return fmt("%.4d", self:get_year()) end,
    -- Time zone offset, the date object is assumed local time (+1000, -0230)
    ['%z']=function(self) local b = -self:get_bias(); local x = abs(b); return fmt("%s%.4d", b < 0 and "-" or "+", fix(x/60)*100 + floor(mod(x,60))) end,
    -- Time zone name, the date object is assumed local time
    ['%Z']=function(self) return self:get_tz_name() end,
    -- Misc --
    -- Year, if year is in BCE, prints the BCE Year representation, otherwise result is similar to "%Y" (1 BCE, 40 BCE)
    ['%\b']=function(self) local x = self:get_year() return fmt("%.4d%s", x>0 and x or (-x+1), x>0 and "" or " BCE") end,
    -- Seconds including fraction (59.998, 01.123)
    ['%\f']=function(self) local x = self:get_fracsec() return fmt("%s%.9f",x >= 10 and "" or "0", x) end,
    -- percent character %
...
--[[ THE LOCAL DATE OBJECT METHODS ]]--
  ...
  function dobj:get_bias()  return (getbiasloc2(self.daynum, self.dayfrc))/SECPERMIN end

  function dobj:get_tz_name()
    local _, tvu, _ = getbiasloc2(self.daynum, self.dayfrc)
    return tvu and osdate("%Z",tvu) or ""
  end

  function dobj:timestamp()
    local y,m,d = breakdaynum(self.daynum)
    local hour,min,sec = breakdayfrc(self.dayfrc)
    return ostime({year=y, month=m+1, day=d, hour=hour, min=min, sec=sec})
  end
...
--#if not DATE_OBJECT_AFX then
  ...
  function date.iso_date(y,w,d) return date_new(makedaynum_isoywd(y + 0, w and (w+0) or 1, d and (d+0) or 1), 0)  end
  function date.set_century_flip(y)
    if y ~= floor(y) or y < 0 or y > 100 then date_error_arg() end
    centuryflip = y
  end
  function date.get_century_flip() return centuryflip end
...
--#end -- not DATE_OBJECT_AFX
  ...
  if tm then
    ...
    DATE_EPOCH = date_epoch and date_epoch:span_days()
