...
-- Private:
-- Parses a line, looking for possible function definitions (in a very na飗e way) 
...
Dumper.new = function(thread)
	...
	if type(thread) == "thread" then
		...
		t.getlocal = function(level, loc)
			...
			return debug.getlocal(level, loc)
		end
	else
...
function Dumper:DumpLocals (level)
	...
	while name do
		...
		elseif type(value) == "table" then
			...
			elseif value.__type == "entity" then
				self:add_f("%s%s = %s\r\n", prefix, name, safe_tostring(value))
			else
				local txt = "{"
...
function _M.stacktrace(thread, message, level)
	...
	level = level or 3 --前3级为stackTracePlus自身信息
...
===============
	...
	while info do
		...
		elseif info.what == "Lua" then
			local source = info.source
			if string.match(source, "stackTracePlus.lua") == nil then
				...
				local function_type = (info.func ~= nil) and "function" or ""
